...
```text
dist/licensed/GenerateSave.exe
```
...
- `licensed`（或不传参数）：生成 `dist\licensed\GenerateSave.exe`，启动时必须校验机器许可证。
- `free`：生成 `dist\free\GenerateSave-Free.exe`，完全跳过许可证校验，窗口标题会标记“免授权版”。该版本一旦转发便可被任何人使用，应仅按需提供给可信人员。
